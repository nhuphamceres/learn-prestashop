...
function upgrade_module_4_2_259($module)
{
    ...
    if (CommonTools::tableExists($marketplace_table)) {
        ...
        if ($result && isset($result, $result['value'])) {
            $storedContexts = unserialize(base64_decode($result['value'])); //TODO: Validation: Use to evaluate base64 encoded values, required
            if (CommonTools::tableExists(_DB_PREFIX_. AmazonConfiguration::$configuration_table, false)) {
                return AmazonConfiguration::updateValue('context', base64_encode(serialize($storedContexts))); //TODO: Validation: Use to evaluate base64 encoded values, required
            }
        }
    }
