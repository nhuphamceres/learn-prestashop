...
if (version_compare(_PS_VERSION_, '1.5', '<') &&
    ...
    file_exists(PS_ADMIN_DIR.'/../classes/AdminTab.php')
) {
...
//    }
    ...
    protected function _getSortParams($id_lang, $order_by, $order_way)
    {
        ...
        if (empty($order_by)) {
            if (version_compare(_PS_VERSION_, '1.5', '<')) {
                $order_by = $cookie->__get($this->table.'Orderby') 
                    ? $cookie->__get($this->table.'Orderby') 
    ...
    protected function parseData($id_lang)
    {
        ...
        if (!file_exists($orderReportFile) || !file_exists($settleReportFile)) {
            die(Tools::displayError(
                'Missing order report file ('.$orderReportFile.') or settle report file ('.$settleReportFile.')'
            ));
        }

        $amazon_report_ws = new AmazonOrdersReports(
            $marketplace_setup['auth'],
            $marketplace_setup['params'],
            null,
            Amazon::$debug_mode
        );
                ...
                if (is_array($filterValue)) { //datetime
                    if (isset($filterValue[0]) && !empty($filterValue[0])) { //from
                        if (!Validate::isDate($filterValue[0])) {
                            $this->errors[] =
                                $this->trans('The \'From\' date format is invalid (YYYY-MM-DD)', array(), 'Admin.Notifications.Error');
                        } else {
    ...
    protected function _collectFiltersFromCookie()
    {
        ...
        foreach ($cookieFilters as $field => $filterValue) {
            ...
            if (array_key_exists($field, $this->fields_list)) {
                $isDateField =
                    isset($this->fields_list[$field]['type'])
                    && $this->fields_list[$field]['type'] == 'datetime';
...
//        dump($offset, $limit);
    ...
    protected function parseSettleReportData($settleReportData, $orderData)
    {
        ...
        foreach ($settleReportData as $key => $settleInfo) {
            ...
            if ($settleInfo['price_type'] == 'Principal' && $settleInfo['price_amount']) {
                $processedSettleData[$orderId]['items'][$orderItemId]['item_settle_price'] =
                    $settleInfo['price_amount'];
            }

            if ($settleInfo['price_type'] == 'Shipping' && $settleInfo['price_amount']) {
                $processedSettleData[$orderId]['items'][$orderItemId]['item_settle_shipping'] =
                    $settleInfo['price_amount'];
            }
    ...
    public function displayListHeader($token = null)
    {
        ...
        $cookie->$pagination = Tools::getValue(
            'pagination',
            isset($cookie->{$this->table.'_pagination'})
                ? $cookie->{$this->table.'_pagination'}
                : $this->_pagination[1]
        );
