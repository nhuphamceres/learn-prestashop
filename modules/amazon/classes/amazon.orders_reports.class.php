...
class AmazonOrdersReports extends AmazonWebService
{
    ...
    const TYPE_FULFILLED_SHIPMENTS  = '_GET_AMAZON_FULFILLED_SHIPMENTS_DATA_';
    ...
    {
        parent::__construct($auth, $from, $marketPlaces, $debug, $cr);
    }
    ...
    public function parseReport($result, $type)
    {
        ...
        $header = explode("\t", str_replace('-', '_', Tools::strtolower($header)));
    ...
    private function _demoFileName($action, $param)
    {
        ...
        switch ($action) {
            case 'reportList':
                ...
                break;
            case 'getReport':
                $file = sprintf('%s/%s-%s-%s-%s.txt', $directory, 'get_report', $merchantId, $param['id'], $param['type']);
                break;
            case 'getOrdersReport':
                $file = sprintf('%s/%s-%s.txt', $directory, 'get_orders_report', $merchantId);
                break;
            default:
                $file = '';
                break;
