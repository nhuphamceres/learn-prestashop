...
class AmazonMultiChannel extends AmazonOrder
{
    ...
    public static function isEligible($id_order)
    {
        ...
        foreach ($products as $product) {
            if (!AmazonTools::validateSKU($product['product_reference'])) {
                self::pd("AmazonTools::validateSKU, invalid SKU ({$product['product_reference']})");
            ...
            if (!($options = AmazonProduct::getProductOptions($product['product_id'], $order->id_lang, $id_product_attribute))) {
                self::pd("No product options available for this product ({$product['product_reference']})");

                return false;
            }

            if (!isset($options['fba']) || !(bool)$options['fba']) {
                self::pd("FBA flag is not set to on ({$product['product_reference']})");
    ...
    public function CancelFulfillmentOrder($order_id, $id_lang, $debug = false)
    {
        ...
        // Amazon has changed multi-channel order id to order id + reference
        $amazon_multi_channel = new AmazonMultiChannel($order_id);
        $result = $amazonAPI->cancelFulfillmentOrder($amazon_multi_channel->getSellerFulfillmentOrderId());
    ...
    public function getFulfillmentOrder($order_id, $id_lang, $debug = false)
    {
        ...
        // Amazon has changed multi-channel order id to order id + reference
        $amazon_multi_channel = new AmazonMultiChannel($order_id);
        $result = $amazonAPI->getFulfillmentOrder($amazon_multi_channel->getSellerFulfillmentOrderId());
    ...
    public function createFulfillmentOrder($id_lang, $debug = false)
    {
        ...
        $AmazonOrder = array(
            'DisplayableOrderDateTime' => gmdate('Y-m-d\TH:i:s\Z', time()),
        ...
        $currencyAndMkp = $this->searchForCurrencyAndMkp($target_country);
        if (is_string($currencyAndMkp)) {
            self::$errors[] = $currencyAndMkp;
            
            return false;
        }
        $target_id_currency = $currencyAndMkp['currency'];

        self::pd(
            "target country: $target_country \n",
            "target id_lang: {$currencyAndMkp['lang']} \n",
        ...
        if ($this->amazon_order_info->is_standard_feature_available) {
            ...
            $this->amazon_order_info->marketplace_id = $currencyAndMkp['mkp'];
        }
        ...
        if (!$result || !is_array($result) || !count($result)) {
            $error = sprintf('Product availability check failed or no items available for order id: %s, %s', $id_order, print_r($result, true));
            self::pd($error);
            self::$errors[] = $error;
            return false;
        }
        ...
        foreach ($result as $Item) {
            ...
            if (isset($Item['InStockSupplyQuantity']) && $Item['InStockSupplyQuantity'] >= $AmazonOrder['Items'][$index]['Quantity']) {
                self::pd(sprintf('Availability Check: %s - Quantity: %s', $Item['SKU'], $Item['InStockSupplyQuantity']));
        ...
        if (!$result) {
            $apiError = $amazonAPI->getResponseError();
            self::$errors[] = sprintf('Amazon Error - Code: %s. Message: %s', $apiError->Code, $apiError->Message);
            return false;
        }
        if (isset($result->ResponseMetadata->RequestId) && preg_match('/([0-9A-Fa-f]{4,16}[\-]{0,}){5}/', (string)$result->ResponseMetadata->RequestId)) {
            ...
            self::$errors[] = sprintf('%s(#%d): Unknown Error, content: {%s}', basename(__FILE__), __LINE__, print_r($result, true));
            return (false);
        }
    }
    ...
    {
        // prevent error "Kernel Container is not available"
