...
$(document).ready(function () {
    ...
    $('#submit-import').click(function () {
        var target_tab = $('#menudiv-import'),
            $langEle = $('input[name="amazon_lang"]');
        ...
        if (!$langEle.is(':checked') && !$langEle.val()) {
            alert($('#msg_lang').val());
            return false;
        }
        ...
        var pAjax = {
            ...
            dataType: 'jsonp',
            url: $('#import_url').val(),
            // &callback=? will be added automatically by jsonp
            data: $('#country-selector').serialize() + '&' + $('#amazonImportOptions').serialize() + '&' + $.param({
    ...
    $('#submit-import-verify').click(function () {
        ...
        var pAjax = {
            ...
            url: $('#import_url').val(),
            // &callback=? will be added automatically by jsonp
            data: $('#country-selector').serialize() + '&' + $.param({
                action: 'get-products',
                instant_token: $('#instant_token').val(),
                context_key: $('#context_key').val(),
                rand: new Date().valueOf(),
