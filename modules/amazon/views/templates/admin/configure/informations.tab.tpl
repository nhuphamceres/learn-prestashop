...
<div id="menudiv-informations" class="tabItem {if $informations.selected_tab}selected{/if} panel form-horizontal">
    ...
    <div class="form-group">
        <label class="control-label col-lg-3">{l s='Amazon Web Services Status' mod='amazon'}</label>{if !$psIsGt15}
    <br/>{/if}
        <div align="left" class="margin-form col-lg-9">

            {if ($informations.display)}
                <div id="status-loader"></div>
                <div id="status-error" class="warn {if $psIsGt15}alert alert-warning{/if}"
            ...
            {else}
                <div class="hint {if $psIsGt15}alert alert-info{/if}">{l s='When your module will be configured, the service status will appear here.' mod='amazon'}</div>
            {/if}
        </div>
    </div>
