...
class AmazonOrdersReport extends Amazon
{
    ...
    public function init()
    {
        ...
        if (!isset($marketPlaceIds[$id_lang]) || !$marketPlaceIds[$id_lang]) {
            ...
            self::$errors[] = $error = sprintf(
                '%s "%s"',
                $this->l('Marketplace is not yet configured for'),
                $lang->name
            );
    ...
    protected function manageReports()
    {
        ...
        $request_fulfilled_shipments = $this->ws->reportList(null, AmazonOrdersReports::TYPE_FULFILLED_SHIPMENTS);

        $this->_printDebug(sprintf(
            '%s(#%d): - Order Report Request List: %s',
            basename(__FILE__),
            __LINE__,
            print_r($request_list_order, true)
        ));
        $this->_printDebug(sprintf(
            '%s(#%d): - Order Settlement Report Request List: %s',
            basename(__FILE__),
            __LINE__,
            print_r($request_list_settlement, true)
        ));
        $this->_printDebug(sprintf(
            '%s(#%d): - Tax Report Request List: %s',
            basename(__FILE__),
            __LINE__,
            print_r($request_list_tax, true)
        ));
        $this->_printDebug(sprintf('%s(#%d): - Order Fulfilled Shipments: %s', basename(__FILE__), __LINE__, print_r($request_fulfilled_shipments, true)));
        ...
        if (!is_array($request_fulfilled_shipments) || !count($request_fulfilled_shipments)) {
            $this->scheduleOrderReport($last_import, AmazonOrdersReports::TYPE_FULFILLED_SHIPMENTS);
        }
        ...
        $this->processReports($request_fulfilled_shipments);
    }
    ...
    protected function processReports($request_list)
    {
        ...
        foreach ($request_list as $requested_report) {
            ...
            if (!$report_id) {
                $this->_printDebug(sprintf(
                    '%s(#%d): - Order Report: Report not yet available',
                    basename(__FILE__),
                    __LINE__
                ));
            } else {
                ...
                if (Tools::strlen($report_data)) {
                    if (AmazonOrdersReports::TYPE_FULFILLED_SHIPMENTS == $report_type) {
                        $this->processReportFulfilledShipments($report_data);
                    } else {
    ...
    protected function processReportAddress($result)
    {
        if ($result == null or empty($result)) {
            AmazonListOrder::$errors[] = sprintf(
                '%s(#%d): processReport - Report is empty !',
                basename(__FILE__),
                __LINE__
            );
            return (false);
        ...
        // 2020-09-29: $result is already in utf-8, utf8_encode() 1 more time will garble the text.
        ...
        $this->pddE(str_repeat('-', 160), sprintf('Orders: %s lines', count($lines)));

        $header = array_shift($lines);
        $headerAmazon = explode("\t", $header);
        $this->pddE(str_repeat('-', 160), sprintf('Header: %s', print_r($headerAmazon, true)));

        $headerPS = array();
        $reportFieldMapping = AmazonAddress::$report_mapping;
        foreach ($headerAmazon as $headerAmazonIndex => $headerAmazonField) {
            if (isset($reportFieldMapping[$headerAmazonField])) {
                $headerPS[$headerAmazonIndex] = $reportFieldMapping[$headerAmazonField];
            }
        }
        $this->pddE(sprintf('Header after mapping: %s', print_r($headerPS, true)));

        $amazon_orders = array();
        foreach ($lines as $line) {
            if (!empty($line)) {
                // Create order line markup based on mapping fields (ensure it has enough require fields)
                $orderLine = array_fill_keys($reportFieldMapping, '');

                $orderExplode = explode("\t", $line);
                if (count($headerAmazon) !== count($orderExplode)) {
                    // Warning
                    $this->pddE(
                        sprintf(
                            'Length mismatch between header and line: %d - %d',
                            count($headerAmazon),
                            count($orderExplode)
                        ),
                        $orderExplode
                ...
                // Line & header mapping
                foreach ($orderExplode as $columnIndex => $orderExplodeValue) {
                    if (isset($headerPS[$columnIndex])) {
                        $orderLine[$headerPS[$columnIndex]] = $orderExplodeValue;
                    }
                }
                $this->pddE(sprintf('Order line mapping: %s', print_r($orderLine, true)));

                $amazon_orders[] = $orderLine;
            }
        }

        if (count($amazon_orders)) {
            ...
            foreach ($amazon_orders as $order) {
                ...
                foreach ($order as $key => $value) {
                    $dbValue = strstr($key, 'date') ? date('Y-m-d H:i:s', strtotime($value)) : $value;
                    $db_values[] = '"' . pSQL($dbValue) . '"';
                }
                $values = implode(',', $db_values);
                $sql = sprintf(
                    'REPLACE INTO `' . _DB_PREFIX_ . Amazon::TABLE_MARKETPLACE_ORDER_ADDRESS . '`(%s)
                        VALUES(%s);',
                    $fields,
                    $values
                );
                $this->pddE(sprintf('SQL: %s', print_r($sql, true)));
    ...
    /**
     * store fulfillment center id to marketplace_orders
     * @param string $response
     *
     * @return bool
     */
    private function processReportFulfilledShipments($response)
    {
    ...
    protected function parseSettleData($data)
    {
        ...
        foreach ($data as $key => $settleInfo) {
            ...
            if ($settleInfo['item_related_fee_type'] == 'ShippingHB' && $settleInfo['item_related_fee_amount']) {
                $processedSettleData[$orderId]['items'][$orderItemId]['item_fee_shipping'] =
                    $settleInfo['item_related_fee_amount'];
            }
        }
     ...
     */
    protected function scheduleOrderReport($last_import, $report_type = null)
    {
        if (strtotime($last_import) < (time() - AmazonOrdersReports::LAG)) {
            if ($report_type == AmazonOrdersReports::TYPE_FULFILLED_SHIPMENTS) {
                $start_date = gmdate('c', strtotime('now -15 days'));
                $report_request_id = $this->ws->reportRequest($start_date, null, $report_type);
            ...
            if (!$report_request_id) {
                ...
                printf(
                    '%s... (%d)',
                    $this->l('An order report has been requested, please wait a while'),
                    $report_request_id
                );
            }
        }
    ...
    {
        $date_min = Db::getInstance()->getValue(
            'SELECT MAX(`date`) 
        ...
        $mp_orders = Db::getInstance()->executeS(
            'SELECT * FROM `'._DB_PREFIX_.Amazon::TABLE_MARKETPLACE_ORDER_ADDRESS.'` 
            WHERE date >= "'.pSQL($date_min).'"'
        );
        ...
        if (is_array($mp_orders) && count($mp_orders)) {
            foreach ($mp_orders as $mp_order) {
                ...
                if (isset($marketplace_addresses->shipping_address) &&
                    $marketplace_addresses->shipping_address instanceof AmazonWsAddress
                ) {
                    if (Amazon::$debug_mode) {
                ...
                $shipping_address_id = $shipping_address->lookupOrCreateamazonAddress(
                    $order->id_lang,
                    $marketplace_addresses->shipping_address
                );
                ...
                if ($shipping_address_id != $order->id_address_delivery) {
                    ...
                    if (Amazon::$debug_mode) {
                        CommonTools::p(sprintf(
                            '%s - %s::%s - line #%d - Delivery Address updated: %d',
                            basename(__FILE__),
                            __CLASS__,
                            __FUNCTION__,
                            __LINE__,
