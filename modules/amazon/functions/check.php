    ...
    public function status()
    {
        ...
        if (!is_array($merchantIds)) {
            ...
            if($amazonEurope){
                ...
                //get master platform lang
                $master_id_lang = 0;
                $eur_id_langs = array();
                
                foreach($marketPlaceIds  as $id_lang => $marketPlaceId){
                    if(in_array($marketPlaceId, $europeanPlatforms)){
                        $eur_id_langs[$id_lang] = $marketPlaceId;
                    }
                }


                foreach($marketPlaceRegions  as $id_lang => $region){
                    if(!isset($eur_id_langs[$id_lang])){
                        //is not part of EU platforms
                        continue;
                    }

                    if($region == $marketPlaceMaster){
                        $master_id_lang = $id_lang;
                        break;
                    }
                }

                if($master_id_lang && isset($merchantIds[$master_id_lang]) && isset($mwsTokens[$master_id_lang])){
                    unset($eur_id_langs[$master_id_lang]);
                    foreach($eur_id_langs as $id_lang=>$marketPlaceId){
                        $merchantIds[$id_lang] = $merchantIds[$master_id_lang];
                        $mwsTokens[$id_lang] = $mwsTokens[$master_id_lang];
                    }
                }
            } //END of Fix for EU platforms
            ...
            foreach ($merchantIds as $id_lang => $merchantId) {
                ...
                $auth = array(
                    ...
                    'mwsToken' => isset($mwsTokens[$id_lang]) ? $mwsTokens[$id_lang] : ''
                );
                ...
                $record['platform'] = 'Amazon '.Tools::strtoupper($marketPlaceRegions[$id_lang]);
                $record['image'] = $this->images.'status_red.png';

                if (isset($result->GetServiceStatusResult)) {
                    $record['status'] = isset($result->GetServiceStatusResult->Status) ? (string)$result->GetServiceStatusResult->Status : $this->l('Unknown');
                    $record['datetime'] = date('Y-m-d H:i:s', (int)strtotime($result->GetServiceStatusResult->Timestamp));

                    switch ($record['status']) {
                        case 'GREEN':
                        case 'GREEN_I':
                            $record['image'] = $this->images.'status_green.png';
                            $status = true && $status;
                            break;
                        case 'YELLOW':
                            $record['image'] = $this->images.'status_yellow.png';
                            $status = true && $status;
                            break;
                        default:
                            $status = false && $status;
                    ...
                    if (isset($result->GetServiceStatusResult->Timestamp)) {
                        $record['drift'] = (int)strtotime($result->GetServiceStatusResult->Timestamp) - $request_time;
                    } else {
                        $record['drift'] = 0;
                    }
                    ...
                    if (isset($result->GetServiceStatusResult->Messages)) {
                        foreach ($result->GetServiceStatusResult->Messages as $message) {
                            $record['messages'] .= (string)$message.nl2br(Amazon::LF);
                        }
                    } else {
                        $record['messages'] .= $this->l('Everything is OK, no message has been returned').nl2br(Amazon::LF);
                    }

                    if ((int)$record['drift'] > 30) {
                        $record['messages'] .= sprintf($this->l('There is a time drift of %d seconds. Amazon allows a maximum of 2 minutes. Please consider this point.').nl2br(Amazon::LF), $record['drift']);
                    }
                } else {
                    $record['status'] = $this->l('Failed');
                    $record['datetime'] = null;
                    $record['drift'] = null;
                    $record['messages'] = $this->l('Unable to connect to');
                }
                $statuses[] = $record;
            }
        }
