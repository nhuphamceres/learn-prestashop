...
class AmazonShippingConfig extends Amazon
{
    ...
    protected function reportRequest()
    {
        ...
        if (!$xml instanceof SimpleXMLElement or isset($xml->Error)) {
            printf('%s(#%d): ReportRequest failed - %s', basename(__FILE__), __LINE__, print_r($xml, 1));
    ...
    protected function reportRequestList()
    {
        ...
        if (!$xml instanceof SimpleXMLElement or isset($xml->Error)) {
            printf('%s(#%d): ReportRequestList failed - %s', basename(__FILE__), __LINE__, print_r($xml, 1));
            return (false);
        }
