...
class AmazonOrdersFix extends Amazon
{
    ...
    public function fixOrders($action)
    {
        ...
        $this->file_orders = sprintf(
            '%s%s_%s_%s.raw',
            $this->import,
            self::MERCHANT_REPORT_NAME,
            $this->merchantId,
            $this->region
        );
        ...
        if ($pass == false && $last_request && $last_request > time() - (self::FILE_MAX_TIME)) {
            // A reports has been requested less than one hour ago we do not request,
            // we check if there is an available report
            //
    ...
    protected function processOrders()
    {
        ...
        if (($result = AmazonTools::fileGetContents($this->file_orders)) === false) {
            self::$errors[] = sprintf(
                '%s(#%d): processOrders - Unable to read input file (%s)',
                basename(__FILE__),
                __LINE__,
                $this->file_orders
            );
        ...
        foreach ($amazonOrders as $SKU => $AmazonQty) {
            $productCheckArray = AmazonProduct::checkProduct($SKU, $this->id_shop, $this->amazon_id_lang);
            $productCheck = $productCheckArray['count'];

            if ($productCheck == 0) {
                //printf(
                    //'%s(#%d): processOrders - SKU/Reference not found in your database. Please check existance of this product: "%s"',
                    //basename(__FILE__),
                    //__LINE__,
                    //$SKU
                //);
                ...
                //printf(
                    //'%s(#%d): processOrders - Unable to import duplicate product "%s" - Please remove the duplicate product in your database.',
                    //basename(__FILE__),
                    //__LINE__,
                    //$SKU
                //);
            ...
            if ($product->id_product_attribute) {
                $combination_options = AmazonProduct::getProductOptions(
                    $id_product,
                    $this->id_lang,
    ...
    protected function reportRequest()
    {
        ...
        if (!isset($xml->RequestReportResult->ReportInfo->ReportProcessingStatus) ||
            !isset($xml->RequestReportResult->ReportInfo->ReportId)
        ) {
            printf('%s(#%d): %s - reportRequest Failed', basename(__FILE__), __LINE__);
    ...
    protected function reportRequestList()
    {
        ...
        if (Amazon::$debug_mode) {
            CommonTools::p(sprintf(
                '%s(#%d): reportList result: %s',
                basename(__FILE__),
                __LINE__,
                nl2br(print_r($xpath_result, true))
            ));
        }
    ...
    protected function getReport($reportId)
    {
        ...
        if (file_put_contents($this->file_orders, $result) === false) {
            printf(
                '%s(#%d): getReport - Unable to write to output file: %s',
                basename(__FILE__),
                __LINE__,
                $this->file_orders
            );
