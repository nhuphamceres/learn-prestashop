...
class AmazonProductsImport extends AmazonProductsReport
{
                ...
                if ($offset && $offset >= $count_offers) {
                    ...
                    if (is_array($items) && count($items)) {
                        ...
                        foreach ($items as $key => $item) {
                            $check_sku = trim(Tools::substr($item['sku'], 0, 32));
                            
                            // skip an existing when only create
                            $productCheckArray = AmazonProduct::checkProduct($check_sku, $this->context->shop->id) ;
                    ...
                    if (is_array($sku_list) && count($sku_list)) {
                        ...
                        if (count($amazon_products)) {
                            ...
                            if (Amazon::$debug_mode) {
                                CommonTools::p(sprintf('%s - %s(#%d): %s - %s: %s'.Amazon::LF, date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $debug, print_r($amazon_product, true)));
                            }
                        }
    ...
    public function importProduct(&$amazon_product)
    {
        ...
        if ($amazon_data['parent_asin'] && isset(self::$variants[$amazon_data['parent_asin']])
            ...
            $has_variant = true;
            $sku = trim(Tools::substr($amazon_data['parent_asin'], 0, 32));
            $child_sku = trim(Tools::substr($amazon_product['sku'], 0, 32));
        } else {
            $has_variant = false;
        ...
        try {
            ...
            self::$errors[] = $debug = 'Throwable Exception: '.  $e->getMessage(). "\n";
    ...
    public function getProducts()
    {
        ...
        if ($this->initDownload()) {
            if ($offers = Tools::getValue('process')) {
                ...
                if (Amazon::$debug_mode) {
                    CommonTools::p(sprintf('%s - %s(#%d): %s - Request Time: "%s", elapsed: %d', date('c'), basename(__FILE__), __LINE__, __FUNCTION__, date('c', $request_time), $elapsed));
                }
                ...
                if ($elapsed > 60 * 60) {
                    ...
                    if (Amazon::$debug_mode) {
                        CommonTools::p($debug);
                        CommonTools::p(sprintf('%s - %s(#%d): %s - ERROR: Request Time: "%s", elapsed: %d - delay expired', date('c'), basename(__FILE__), __LINE__, __FUNCTION__, date('c', $request_time), $elapsed));
                    }
                    ...
                    if ($reportRequestId) {
                        ...
                        if (Amazon::$debug_mode) {
                            CommonTools::p(sprintf('%s - %s(#%d): %s - A report has been already requested and there is not any available report yet', date('c'), basename(__FILE__), __LINE__, __FUNCTION__));
                        }
                ...
                if ($reportRequestId = $this->reportRequest()) {
                    ...
                    if (Amazon::$debug_mode) {
                        CommonTools::p(sprintf('%s - %s(#%d): %s - Report Request ID: "%s"', date('c'), basename(__FILE__), __LINE__, __FUNCTION__, $reportRequestId));
                    }
                    $continue = true;
                    $pass = true;
    ...
    protected function processOffersInventory($display = true)
    {
        ...
        if (Amazon::$debug_mode) {
            printf('%s - %s(#%d): processOffersInventory()'.nl2br(Amazon::LF), date('c'), basename(__FILE__), __LINE__, __FUNCTION__);
        }
        ...
        if ($fileContent === false) {
            ...
            self::$errors[] = $error;
            $this->edd($error, __LINE__, true);
            return (false);
        }
        try {
            $fileContent = mb_convert_encoding($fileContent, 'UTF-8', mb_detect_encoding($fileContent, 'UTF-8, ISO-8859-1', true));
        } catch (Exception $exception) {
            $this->edd('WARNING: Cannot convert file content to UTF-8, process may have many problems!', __LINE__, true);
            $this->edd(sprintf('Code: %s. Error: %s', $exception->getCode(), $exception->getMessage()), __LINE__, true);
        }
