 ...
 *
 * @author    Olivier B.
 * @copyright Copyright (c) 2011-2016 Common Services Co Ltd - 90/25 Sukhumvit 81 - 10260 Bangkok - Thailand
 * @license   Commercial license
 * Support by mail:  support.mirakl@common-services.com
 */

class MiraklPickupPoint
{
    /* Common variables */
    ...
    public static function getMondialRelayCarriedId($method)
    {
        $id_carrier = false;

        if (!Tools::strlen($method)) {
            return(false);
        }

        $mr_carriers = Db::getInstance()->executeS(
            'SELECT c.`id_carrier` FROM `'._DB_PREFIX_.'carrier` c
            LEFT JOIN `'._DB_PREFIX_.'mr_method` mrm ON (c.`id_carrier` = mrm.`id_carrier`)
            WHERE `external_module_name` IN ("mondialrelay", "mondialrelayadvanced") AND `deleted` = 0 AND mrm.`dlv_mode` = "'.pSQL($method).'"'
        );

        if (is_array($mr_carriers) && count($mr_carriers)) {
            foreach ($mr_carriers as $mr_carrier) {
                $id_carrier = $mr_carrier['id_carrier'];
            }
        }
    ...
    public static function isMondialRelayInstalled()
    {
        ...
        $mr_carriers = Db::getInstance()->executeS('SELECT id_carrier FROM `'._DB_PREFIX_.'carrier` WHERE `external_module_name` IN ("mondialrelay", "mondialrelayadvanced") AND `deleted` = 0');
    ...
    // 2021-05-07: Remove unused getMondialRelayMarketplaceOrdersStatesByIdLang()

    public function save()
    {
        switch ($this->pickup_type) {
            case (self::MONDIAL_RELAY_TYPE):
                return $this->saveMondialRelay();
            case (self::SO_COLISSIMO_TYPE):
                return $this->saveSoColissimo();
            default:
                return null;
        }
    }

    private function saveMondialRelay()
    {
        $params = array(
    ...
    private function saveSoColissimo()
    {
        $params = array(
            'id_cart' => (int)$this->id_cart,
            'id_customer' => (int)$this->id_customer,
