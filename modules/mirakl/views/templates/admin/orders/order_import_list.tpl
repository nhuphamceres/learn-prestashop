...
<tr class="order-item">
    {if $id_order}
        <td align="center">
            <img src="{$image_path|escape:'htmlall':'UTF-8'}icon_valid_16.png" title="{l s='Already Imported' mod='mirakl'}" />
        </td>
        <td>
            <a href="#" class="order_link"
               onclick="window.open('?tab=AdminOrders&id_order={$id_order|escape:'htmlall':'UTF-8'}&vieworder&token={$token_order|escape:'htmlall':'UTF-8'}')">
                {$oid|escape:'htmlall':'UTF-8'}({$id_order|escape:'htmlall':'UTF-8'})
            </a>
        </td>
    {else}
        <td align="center">
            <input type="checkbox" name="selected_orders[]" {if $disabled}disabled{/if} {if $checked}checked{/if}
    ...
    <td>{$customer_id|escape:'htmlall':'UTF-8'}</td>
    <td>{$payment_type|escape:'htmlall':'UTF-8'}</td>
    ...
    <td colspan="8">
        <table class="import-table">
            ...
            {foreach from=$details item="detail"}
                <tr>
                    {if $detail.pass && $detail.existing}
                        <td style="padding-left:8px;width:20px">
                            <img src="{$image_path|escape:'htmlall':'UTF-8'}icon_valid_16.png" title="{l s='Already Imported' mod='mirakl'}" />
                        </td>
                    {elseif $detail.pass && !$detail.ps_qty}
                        <td style="padding-left:8px;width:20px">
                            <img src="{$image_path|escape:'htmlall':'UTF-8'}soos.png" title="{l s='Out of Stock' mod='mirakl'}" />
                        </td>
                    {elseif $detail.pass}
                    ...
                    {else}
                        <td style="padding-left:8px;width:20px">
                            <img src="{$image_path|escape:'htmlall':'UTF-8'}cross.png" title="{l s='Product not found' mod='mirakl'}" />
                        </td>
                    {/if}

                    <td>{$detail.products_row|escape:'htmlall':'UTF-8'}</td>
                    <td>{$detail.product_sku|escape:'htmlall':'UTF-8'}</td>
                    <td>{$detail.product_title|escape:'htmlall':'UTF-8'}</td>
                    <td>{$detail.offer_sku|escape:'htmlall':'UTF-8'}</td>
                    <td>{$detail.orderLineState|escape:'htmlall':'UTF-8'}</td>
