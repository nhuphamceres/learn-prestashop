...
class PriceMinisterCategories
{

    const CONF_TYPE = 'pm_category';
    const MAIN_GROUP = 'category';

    public static $table = PriceMinister::TABLE_PRICEMINISTER_CONFIGURATION;

    /* receives profile as it's obtained */
    public static function save($pm_categories)
    {
        ...
        foreach ($pm_categories as $key => $pm_category) {
            $inserts[] = '('.pSQL($key).', "'.
                pSQL(self::CONF_TYPE).'", "'.
    ...
    public static function getCategoryName($id_category, $context = null)
    {
        if (!$context) {
        ...
        $sql = 'SELECT `name` FROM `'._DB_PREFIX_.'category_lang`
				...
				AND '.(version_compare(_PS_VERSION_, '1.5', '>=') ? '`id_shop` = '.(int)$context->shop->id : '1');
